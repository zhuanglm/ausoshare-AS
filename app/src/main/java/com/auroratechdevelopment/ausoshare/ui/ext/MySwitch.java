...
/**
 * Created by Raymond Zhuang on 2016-06-29.
 */
...
public class MySwitch extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.MySwitch);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.MySwitch_zThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.MySwitch_zThumbColor);
            margin = ta.getDimension(R.styleable.MySwitch_zThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.MySwitch_zThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.MySwitch_zThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.MySwitch_zThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.MySwitch_zThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.MySwitch_zThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.MySwitch_zThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.MySwitch_zThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.MySwitch_zBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.MySwitch_zBackDrawable);
            backColor = ta.getColorStateList(R.styleable.MySwitch_zBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.MySwitch_zBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.MySwitch_zAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.MySwitch_zFadeBack, true);
            tintColor = ta.getColor(R.styleable.MySwitch_zTintColor, tintColor);
            textOn = ta.getString(R.styleable.MySwitch_zTextOn);
            textOff = ta.getString(R.styleable.MySwitch_zTextOff);
            textMarginH = ta.getDimension(R.styleable.MySwitch_zTextMarginH, textMarginH);
            ta.recycle();
        }
